...
public class RocketMQOptions {
    ...
    /**
     * rocketmq v4 endpoints means nameserver address
     * rocketmq v5 endpoints means proxy server address
     */
    ...
    public static final ConfigOption<AccessChannel> OPTIONAL_ACCESS_CHANNEL =
            ConfigOptions.key("accessChannel").enumType(AccessChannel.class).defaultValue(AccessChannel.CLOUD)
    ...
    public static final ConfigOption<Integer> CLIENT_CALLBACK_EXECUTOR_THREADS =
            ConfigOptions.key("clientCallbackExecutorThreads")
                    .intType().defaultValue(Runtime.getRuntime().availableProcessors())
                    .withDescription("The number of processor cores " +
    ...
    public static final ConfigOption<Long> PARTITION_DISCOVERY_INTERVAL_MS =
            ConfigOptions.key("partitionDiscoveryIntervalMs")
                    .longType().defaultValue(10000L)
                    .withDescription("Time interval for polling route information from nameserver or proxy");

    public static final ConfigOption<Long> HEARTBEAT_INTERVAL =
            ConfigOptions.key("heartbeatInterval")
                    .longType().defaultValue(30000L)
                    .withDescription("Interval for regularly sending registration heartbeats to broker");
    ...
    public static final ConfigOption<Boolean> VIP_CHANNEL_ENABLED =
            ConfigOptions.key("vipChannelEnabled").booleanType().defaultValue(false)
                    .withDescription("Whether to enable vip netty channel for sending messages");

    public static final ConfigOption<Boolean> USE_TLS =
            ConfigOptions.key("useTLS").booleanType().defaultValue(false)
    ...
    public static final ConfigOption<LanguageCode> LANGUAGE_CODE =
            ConfigOptions.key("language")
                    .enumType(LanguageCode.class).defaultValue(LanguageCode.JAVA)
